/* ============ LENTE TREINO: plano A–E, variações, regras de carga, dor, deload e preferências ============ */
...
LENS_R.treino = () => {
  ...
  return head + main + prefsSec(vol) + painSec();
};
...
ACT.pickday = b => { UI.pickDay = b.dataset.day; rLens(); };
ACT["sel-var"] = b => { if (b.dataset.v) S.sel[b.dataset.s] = b.dataset.v; else delete S.sel[b.dataset.s]; save(); rLens(); };
