...
const FRASES = [
  ...
  ["Manter é o treino mais difícil. Por isso é o que mais vale.", ""],
  ["Você já perdeu 45 kg uma vez. O que falta agora é ficar.", ""],
  ["Disciplina é escolher o que você quer mais, não o que quer agora.", ""],
  ["Às 5h ninguém está olhando. É aí que o resultado é feito.", ""],
  ["Um dia ruim não apaga uma semana boa. Volte ao próximo hábito.", ""],
  ["Zero açúcar hoje. Só hoje. Amanhã a gente vê amanhã.", ""],
  ["Não precisa ser perfeito. Precisa ser de novo.", ""],
  ["Cada série é um voto no homem que você está virando.", ""],
  ["Consistência bate intensidade. Sempre bateu.", ""],
  ["Seu corpo ouve tudo o que você faz, todos os dias.", ""],
  ["Caiu? Levanta sem drama. É assim que se mantém o peso.", ""],
  ["Peso é número. Hábito é identidade.", ""],
  ["O ombro pede cuidado, não desistência. Carga certa, técnica limpa.", ""],
  ["Você não está começando do zero. Está começando com experiência.", ""],
  ["Beba água agora. É o hábito mais barato que existe.", ""],
  ["Quem chega em 105 kg é quem aparece na terça-feira chuvosa.", ""],
  ["Comer bem hoje é um presente para a sua versão de amanhã.", ""],
  ["Pequeno e todo dia vence grande e de vez em quando.", ""],
  ["A meta são 5 dias por semana. Não 7. Respire e siga.", ""],
  ["Você não precisa de motivação. Precisa do próximo passo pequeno.", ""],
  ["Raízes antigas crescem devagar, e por isso seguram a árvore.", ""],
  ["Treinar cansado também conta. Talvez conte mais.", ""],
  ["Sono é treino. Durma às 21h30 e acorde inteiro.", ""],
  ["Proteína, água, sono, passos. O básico bem feito derrota o truque.", ""],
  ["Orgulho de ontem não move a barra de hoje. Vá lá e faça.", ""],
  ["Se está difícil, é porque está mudando.", ""],
  ["Sua saúde é a sua conquista mais silenciosa e mais importante.", ""],
  ["Meça o progresso em semanas, não em espelhadas.", ""],
  ["Ficar firme é a parte que ninguém aplaude. Faça mesmo assim.", ""]
];
function fraseIdx() {
...
function rAgua() {
  ...
  const bars = Array.from({ length: 7 }, (_, i) => { const kk = addDays(k, i - 6), v = D(kk).water || 0, f = Math.min(1, v / goal); return `<div class="d ${kk === k ? "t" : ""}"><i style="height:${Math.max(6, f * 100)}%;background:${v >= goal ? "var(--ok)" : v ? "#5aa9d6" : "var(--surface2)"}"></i><span>${DOW[parseKey(kk).getDay()][0]}</span></div>`; }).join("");
  ...
  <section class="card">
    <div class="row between"><div class="sec-t"><span class="dot ok"></span><span class="lbl">Últimos 7 dias</span></div><span class="pill ${ws ? "ok" : ""}">${ws} ${ws === 1 ? "dia" : "dias"} na meta</span></div>
    <div class="week">${bars}</div>
    <p class="muted" style="font-size:15px">Média dos últimos 7 dias: ${(wkAvg / 1000).toFixed(2).replace(".", ",")} L. Barra verde = meta batida.</p>
  </section>
  <section class="card">
