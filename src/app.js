...
/* ============ estado e persistÃªncia (3 camadas) ============ */
const KEY = "sir_v1";
const defaults = () => ({
  v: 1,
...
function prefill(slot, vid) {
  ...
  const out = []; let hint = "Sem histÃ³rico: escolha uma carga leve.";
  if (last) {
    ...
    hint = `Ãšltima vez (${dispDate(last.date)}): ${last.sets.map(s => `${s.kg}Ã—${s.reps}`).join(" Â· ")} â†’ ${nx.dir === "up" ? `hoje +${nx.inc} kg` : nx.dir === "down" ? `hoje âˆ’${nx.inc} kg` : "mantenha a carga"}. ${nx.why}`;
...
/* ============ UI base ============ */
const UI = { tab: "deck", cat: "Todos", q: "", meal: "CafÃ© da manhÃ£", supSort: "price", supType: "all", pickDay: null };
let toastT = null;
...
function orig_rDeck() {
  ...
  if (S.cur) ban.push(banner("acc", "play", "Treino em andamento", `VocÃª comeÃ§ou o treino ${S.cur.day}. Continue de onde parou.`, `<div style="margin-top:8px"><button class="btn sm solid" data-act="wk-resume">Continuar treino</button></div>`));
  const pa = painAvg(); if (pa !== null && pa >= 4) ban.push(banner("bad", "shield", "Dor no ombro em alta", `MÃ©dia das 3 Ãºltimas: ${r1(pa)}/10. Procure um fisioterapeuta antes de aumentar carga de empurrar.`));
  if (deloadSignal()) ban.push(banner("warn", "info", "Sinal de deload", "3 ou mais exercÃ­cios falharam 2 sessÃµes seguidas com a mesma carga. Reduza cerca de 20% nesta semana e reavalie."));
  const bk = S.settings.lastBackup, bdays = bk ? diffDays(k, bk) : (Object.keys(S.days).length ? diffDays(k, firstDay()) : 0);
  if (bdays > 21) ban.push(banner("warn", "download", "Backup atrasado", `${bk ? `Ãšltimo backup hÃ¡ ${bdays} dias.` : "VocÃª nunca exportou um backup."} O arquivo .json Ã© o Ãºnico que sobrevive Ã  troca de celular.`, `<div style="margin-top:8px"><button class="btn sm" data-act="export">Exportar agora</button></div>`));
  const isMon = parseKey(k).getDay() === 1, wk = mondayOf(k);
  if (isMon && !S.weights.some(w => w.d >= wk)) ban.push(banner("acc", "scale", "Dia de pesagem", "Segunda: ao acordar, depois do banheiro, antes de comer. MeÃ§a a cintura junto.", `<div style="margin-top:8px"><button class="btn sm solid" data-act="weigh">Registrar peso</button></div>`));
  ...
  const habits = HABITS.map(([id, t]) => `<button class="chk ${d.h[id] ? "on" : ""} ${id === "acucar" ? "key" : ""}" data-act="habit" data-id="${id}"><span class="box">${ic("check")}</span><span class="t">${t}${id === "acucar" ? `<span class="s">Regra nÂº 1. Se cumprir uma sÃ³, que seja esta.</span>` : ""}</span></button>`).join("");

  const doneToday = !!D(k).wk && !S.cur;
  let wk2;
  if (L && doneToday) {
    const nl = seqNext();
    wk2 = `<div class="sec-t"><span class="dot ok"></span><span class="lbl">Treino de hoje</span></div><h3 class="mid">${L} Â· ${PLAN[L].name} concluÃ­do</h3><p class="muted">As cargas do prÃ³ximo treino jÃ¡ foram atualizadas. PrÃ³ximo: <b>${nl} Â· ${PLAN[nl].name}</b>.</p><button class="btn full" data-act="goto-treino" data-day="${nl}">Ver prÃ³ximo treino</button>`;
  } else if (L) {
    const p = PLAN[L];
    wk2 = `<div class="row between"><div><div class="sec-t"><span class="dot"></span><span class="lbl" style="color:var(--accent-ink)">SessÃ£o do amanhecer Â· ${DOW[parseKey(k).getDay()]}</span></div><h3 class="mid" style="margin-top:4px">${L} Â· ${p.name}</h3></div><span class="pill">05:00 Â· ${estMin(L)} min</span></div>
      <p class="muted">${p.focus}. ${p.cuff ? "ComeÃ§a com aquecimento de manguito." : "Sem aquecimento de manguito hoje."} ${inAdapt() ? "Semana " + weekNo() + " de adaptaÃ§Ã£o: carga leve." : ""}</p>
  ...
  } else wk2 = `<div class="sec-t"><span class="dot sage"></span><span class="lbl">Descanso ativo</span></div><h3 class="mid">Fim de semana</h3><p class="muted">Sem treino de academia. Caminhada livre de 30 minutos conta como hÃ¡bito. Se quiser treinar, escolha um dia na aba Treino.</p>`;
  ...
  <section class="card hero" style="gap:8px">
    <div class="row between"><span class="pill">${DOW[parseKey(k).getDay()]} Â· ${dispDate(k)}</span><span class="lbl">Semana ${weekNo()}${inAdapt() ? " Â· adaptaÃ§Ã£o" : ""}</span></div>
    ...
    <p class="quote">"Still I Rise. O crescimento Ã© orgÃ¢nico, como raÃ­zes antigas que se aprofundam calmas na terra morna."</p>
  </section>
  ...
  <section class="card" style="order:4">
    <div class="row between"><div class="sec-t"><span class="dot sage"></span><span class="lbl">Ritmo semanal</span></div><span class="pill ok">NÃ£o-punitivo</span></div>
    <div class="row" style="gap:16px">
      ...
      <div class="grow" style="display:flex;flex-direction:column;gap:10px">
        <div class="tile row between"><div><div class="lbl">SequÃªncia de dias</div><div class="mid tabnum">${ds}</div></div>${ic("flame")}</div>
        <div class="tile row between"><div><div class="lbl">SequÃªncia de semanas</div><div class="mid tabnum">${ws}</div></div>${ic("trend")}</div>
      </div>
    </div>
    <div class="week">${bars}</div>
    <p class="muted" style="font-size:15px">${wtxt} Dia ativo = zero aÃ§Ãºcar + 2 outros hÃ¡bitos. Semana verde: 5+ dias. Semana amarela (4) nÃ£o quebra a sequÃªncia. SÃ³ 2 semanas ruins seguidas zeram.</p>
  </section>
  <section class="card" style="order:3">
    <div class="row between"><div class="sec-t"><span class="dot ok"></span><span class="lbl">HÃ¡bitos de hoje</span></div><span class="pill">${habitCount(k)}/6</span></div>
  ...
  <section class="card" style="order:5">
    <div class="row between"><div class="sec-t"><span class="dot" style="background:var(--sage)"></span><span class="lbl">HidrataÃ§Ã£o</span></div><span class="lbl">Meta ${S.settings.waterGoal / 1000} L</span></div>
    ...
    <div class="grid3"><button class="btn sm" data-act="water" data-ml="250">+250 ml</button><button class="btn sm" data-act="water" data-ml="500">+500 ml</button><button class="btn sm ghost" data-act="water" data-ml="-250">âˆ’250</button></div>
  </section>
  <section class="card" style="order:6">
    <div class="row between"><div class="sec-t"><span class="dot"></span><span class="lbl">NutriÃ§Ã£o de hoje</span></div><button class="lbl" style="color:var(--accent-ink)" data-go="comer">Registrar comida</button></div>
  ...
  <details class="fold card" style="order:7;padding:0 14px"><summary><span class="row"><span class="dot sage"></span><span class="lbl">Suplementos de hoje Â· ${Object.values(d.s).filter(Boolean).length}/${SUPP_BASE.length}</span></span>${ic("down")}</summary><div class="body">${sup}</div></details>
  ${nextM ? `<section class="card flat" style="order:8"><div class="lbl">PrÃ³ximo marco</div><div class="mid">${nextM.nm} Â· ${nextM.lbl(S.profile.startWeight)}</div><p class="muted">${nextM.txt}</p></section>` : ""}
...
function rTreino() {
  ...
  const list = p.ex.map(e => { const v = findV(e, varId(e)); return `<div class="li"><div class="grow"><div style="font-weight:700">${v[1]}</div><div class="muted" style="font-size:14px">${e.sets} Ã— ${e.reps[0] === e.reps[1] ? e.reps[0] : e.reps[0] + "â€“" + e.reps[1]}${e.unit ? " s" : ""} Â· descanso ${e.rest}s${e.inc ? ` Â· +${e.inc} kg` : ""}</div></div>${nextBadge(e, v[0])}<span class="tag">${e.v.length} variaÃ§Ãµes</span></div>`; }).join("");
  const warm = p.cuff ? CUFF.map(c => `<div class="li"><div class="grow"><div style="font-weight:700">${c.name}</div><div class="muted" style="font-size:14px">${c.sets} Ã— ${c.reps[0]}</div></div><span class="tag">AQUEC.</span></div>`).join("") : "";
  const pa = painAvg();
  $("#v-treino").innerHTML = `
  <section class="card" style="background:transparent;box-shadow:none;padding:0"><h2 class="h1">Treino</h2><p class="muted">Segunda a sexta Ã s 5h, seguindo a sequÃªncia A a E: se vocÃª faltar um dia, o treino continua de onde parou. As cargas da prÃ³xima vez sÃ£o recalculadas ao fim de cada treino.</p></section>
  ...
  ${pa !== null && pa >= 4 ? banner("bad", "shield", "Dor no ombro em alta", `MÃ©dia das 3 Ãºltimas: ${r1(pa)}/10. Procure um fisioterapeuta.`) : ""}
  <div class="chips">${DAY_ORDER.map(x => `<button class="chip ${x === L ? "on" : ""}" data-act="pickday" data-day="${x}">${x} Â· ${PLAN[x].name}</button>`).join("")}</div>
  <section class="card">
    <div class="row between"><div><div class="lbl" style="color:var(--accent-ink)">Treino ${L}</div><h3 class="mid">${p.name}</h3></div><span class="pill">â‰ˆ ${estMin(L)} min</span></div>
    <p class="muted">${p.focus}. Termina com 10 min de esteira inclinada (4â€“6%), ritmo de conversa.</p>
  ...
  <section class="card flat">
    <div class="lbl">BalanÃ§o semanal de sÃ©ries</div>
    ...
    <p class="muted" style="font-size:15px">Puxar â‰¥ empurrar protege o ombro. Aquecimento de manguito antes de peito, ombro e braÃ§o.</p>
  </section>
  <section class="card flat">
    <div class="lbl">Regras de carga</div>
    <p class="muted" style="font-size:16px"><b>Semanas 1â€“2:</b> adaptaÃ§Ã£o com carga leve. <b>Depois, ao fim de cada treino:</b> fechou todas as repetiÃ§Ãµes em todas as sÃ©ries, sobe a carga; falhou na 1Âª sÃ©rie ou em 2 ou mais sÃ©ries, desce um degrau; senÃ£o mantÃ©m. <b>Deload sÃ³ por sinal</b> (2 sessÃµes falhando ou dor â‰¥ 4).<br><b>Regra do RIR:</b> termine sentindo que faria mais 3. Se faria mais de 4, estava leve. Se nÃ£o completou, estava pesada.<br>${INCS_TXT}</p>
    <div class="row between"><span class="grow">RodÃ­zio automÃ¡tico de variaÃ§Ãµes (a cada 4 semanas)</span><button class="tog ${S.settings.rotate ? "on" : ""}" data-act="tog-rotate" aria-label="alternar"><i></i></button></div>
  </section>`;
}
...
function wkRender() {
  ...
  if (step.t === "warm") {
    body = `<span class="pill acc">AQUECIMENTO DE MANGUITO</span><h2 class="mid">${step.c.name}</h2>
      ...
      <div class="lbl">${step.c.sets} sÃ©ries de ${step.c.reps[0]} repetiÃ§Ãµes, carga leve</div>
      <div style="display:flex;flex-direction:column;gap:8px">${dd.sets.map((s, i) => `<button class="chk ${s.done ? "on" : ""}" data-act="wk-warmset" data-i="${i}"><span class="box">${ic("check")}</span><span class="t">SÃ©rie ${i + 1}</span></button>`).join("")}</div>`;
  } else {
    ...
    const rows = dd.sets.map((s, i) => `<div class="setrow ${s.done ? "done" : ""} ${i === pend && c.mode === "set" ? "cur" : ""}"><span class="n">${i + 1}</span>
      <input inputmode="decimal" data-f="kg" data-i="${i}" value="${s.kg || ""}" placeholder="kg" aria-label="carga sÃ©rie ${i + 1}">
      <input inputmode="numeric" data-f="reps" data-i="${i}" value="${s.reps}" aria-label="${unit} sÃ©rie ${i + 1}">
    ...
    const adj = c.mode === "set" && pend >= 0 ? `<div class="chips" style="justify-content:center" aria-label="ajuste rÃ¡pido da prÃ³xima sÃ©rie">${kstep ? `<button class="chip" data-act="wk-adj" data-f="kg" data-d="${-kstep}">âˆ’ ${kstep} kg</button><button class="chip" data-act="wk-adj" data-f="kg" data-d="${kstep}">+ ${kstep} kg</button>` : ""}<button class="chip" data-act="wk-adj" data-f="reps" data-d="${-rstep}">âˆ’ ${rstep} ${ru}</button><button class="chip" data-act="wk-adj" data-f="reps" data-d="${rstep}">+ ${rstep} ${ru}</button></div>` : "";
    body = `<div class="row between"><span class="pill">${slot.sets} Ã— ${slot.reps[0] === slot.reps[1] ? slot.reps[0] : slot.reps[0] + "â€“" + slot.reps[1]}${slot.unit ? " s" : ""} Â· descanso ${slot.rest}s</span><button class="chip" data-act="wk-mode">${c.mode === "set" ? "1 toque por sÃ©rie" : "Registrar ao final"}</button></div>
      <h2 class="mid">${v[1]}</h2>
      ...
      ${rows}
      ${adj}
      ${c.mode === "end" ? `<button class="btn solid full" data-act="wk-allset">${ic("check")} REGISTRAR EXERCÃCIO (${dd.sets.length} sÃ©ries)</button>` : ""}
      <details class="fold" open><summary>Como fazer ${ic("down")}</summary><div class="body"><p class="how">${v[2]}</p></div></details>
      <div><div class="lbl" style="margin-bottom:6px">VariaÃ§Ãµes (para nÃ£o enjoar)</div><div class="chips">${slot.v.map(x => `<button class="chip ${x[0] === dd.vid ? "on" : ""}" data-act="wk-var" data-v="${x[0]}">${x[1].replace(/ \(.*\)/, "")}</button>`).join("")}</div></div>
      ${slot.id === "a2" && dd.vid === "a2_incl_barra" && painAvg() !== null && painAvg() >= 2 ? banner("warn", "shield", "AtenÃ§Ã£o ao ombro", "Sua dor recente estÃ¡ acima de 2. Prefira halteres hoje.") : ""}
      <p class="muted" style="font-size:14px">${done}/${dd.sets.length} sÃ©ries. Termine sentindo que faria mais 3 repetiÃ§Ãµes.</p>`;
  }
  const pIdx = step.t === "ex" ? dd.sets.findIndex(s => !s.done) : -1;
  const cta = step.t === "ex" && c.mode === "set" && pIdx >= 0
    ? `<button class="btn solid big-cta grow" data-act="wk-set" data-i="${pIdx}">${ic("check")} SÃ‰RIE ${pIdx + 1} FEITA</button><button class="btn" data-act="wk-next" aria-label="${last ? "finalizar treino" : "prÃ³ximo exercÃ­cio"}">${ic(last ? "check" : "right")}</button>`
    : `<button class="btn solid big-cta grow" data-act="wk-next">${last ? "FINALIZAR TREINO" : "PRÃ“XIMO"} ${ic(last ? "check" : "right")}</button>`;
  const pct = (c.i + 1) / st.length * 100;
  $("#wk-body").innerHTML = `
    <div class="row between"><button class="circ-btn" data-act="wk-back" aria-label="voltar">${ic("left")}</button><div class="grow" style="text-align:center"><div class="lbl">Treino ${c.day} Â· ${PLAN[c.day].name}</div><div class="muted" style="font-size:14px">${c.i + 1} de ${st.length}</div></div><button class="circ-btn" data-act="wk-exit" aria-label="sair">${ic("x")}</button></div>
...
function restDraw() {
  ...
  <div class="grid2" style="width:100%"><button class="btn" data-act="rest-adj" data-s="-15">âˆ’15 s</button><button class="btn" data-act="rest-adj" data-s="15">+15 s</button><button class="btn" data-act="rest-pause">${RT.paused !== null ? "Retomar" : "Pausar"}</button><button class="btn solid" data-act="rest-skip">Pular</button></div>`;
}
...
function restDone() {
  ...
  notify("Descanso acabou", RT.next || "Hora da prÃ³xima sÃ©rie.");
...
function foodSheet(i) {
  const f = FOODS[i], sug = f[5] === "Bebidas" && /COM aÃ§Ãºcar/.test(f[0]);
  const chips = ["CafÃ© da manhÃ£", "AlmoÃ§o", "Lanche", "Jantar", "Ceia"].map(m => `<button class="chip ${UI.meal === m ? "on" : ""}" data-act="meal-sel" data-m="${m}">${m}</button>`).join("");
  openSheet(`<h3 class="mid">${esc(f[0])}</h3><div class="muted">${f[1]} kcal e ${f[2]} g de proteÃ­na por 100 g Â· fonte ${f[6] === "T" ? "TACO 4Âª ed." : "estimativa/rÃ³tulo"}</div>
  ${sug ? banner("warn", "info", "Bebida com aÃ§Ãºcar", "Quebra a Regra nÂº 1 de hoje. Pode registrar: o app nÃ£o julga, amanhÃ£ continua normal.") : ""}
  <div class="row"><div class="stepper grow"><button data-act="g-adj" data-s="-10">âˆ’</button><input id="g" inputmode="numeric" value="${f[4]}" data-i="${i}"><button data-act="g-adj" data-s="10">+</button><span class="muted">g</span></div></div>
  <div class="grid3"><button class="chip" data-act="g-set" data-g="${f[4]}">1 porÃ§Ã£o</button><button class="chip" data-act="g-set" data-g="${f[4] * 2}">2 porÃ§Ãµes</button><button class="chip" data-act="g-set" data-g="100">100 g</button></div>
...
function gSum(i) { const f = FOODS[i], g = num($("#g").value); $("#g-sum").textContent = `${Math.round(f[1] * g / 100)} kcal Â· ${r1(f[2] * g / 100)} g prot`; }

/* ============ EVOLUÃ‡ÃƒO ============ */
function chartSVG() {
  const pts = S.weights.slice(-16); if (pts.length < 2) return `<p class="muted">O grÃ¡fico aparece a partir da segunda pesagem.</p>`;
  ...
  return `<svg class="svg-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="GrÃ¡fico de peso"><line x1="${pl}" x2="${W - pr}" y1="${y(goal)}" y2="${y(goal)}" stroke="var(--ok)" stroke-dasharray="4 4"/><text x="${pl + 2}" y="${y(goal) - 3}">meta ${goal}</text><path d="${path}" fill="none" stroke="var(--accent)" stroke-width="2.5" stroke-linejoin="round"/>${pts.map((p, i) => `<circle cx="${x(i)}" cy="${y(p.kg)}" r="3.5" fill="var(--accent)"/>`).join("")}<text x="0" y="${y(mx) + 8}">${mx}</text><text x="0" y="${y(mn)}">${mn}</text><text x="${pl}" y="${H - 4}">${dispDate(pts[0].d)}</text><text x="${W - pr}" y="${H - 4}" text-anchor="end">${dispDate(pts[pts.length - 1].d)}</text></svg>`;
}
function rEvol() {
  ...
  $("#v-evol").innerHTML = `
  <section class="card" style="background:transparent;box-shadow:none;padding:0"><h2 class="h1">EvoluÃ§Ã£o</h2><p class="muted">Pesagem semanal, segunda ao acordar. O pescoÃ§o clareia antes da balanÃ§a.</p></section>
  <section class="card">
    <div class="row between"><div><div class="lbl">Peso atual</div><div class="big tabnum">${r1(w)} <span class="muted" style="font-size:16px">kg</span></div></div><div style="text-align:right"><div class="lbl">Desde o inÃ­cio</div><div class="mid tabnum" style="color:${w <= s ? "var(--ok)" : "var(--bad)"}">${r1(w - s) > 0 ? "+" : ""}${r1(w - s)} kg</div></div></div>
    <div class="bar acc"><i style="width:${prog * 100}%"></i></div><div class="row between"><span class="lbl">${s} kg</span><span class="lbl">Meta ${g} kg Â· faltam ${r1(Math.max(0, w - g))}</span></div>
    ${chartSVG()}
    <button class="btn solid full" data-act="weigh">${ic("scale")} REGISTRAR PESO</button>
    <p class="muted" style="font-size:15px">Nunca se pese depois de furar: o nÃºmero vem inflado por sÃ³dio e Ã¡gua e vocÃª vai ler como gordura.</p>
    ${S.weights.length ? `<div class="list">${S.weights.slice(-6).reverse().map(x => `<div class="li"><div class="grow"><b class="tabnum">${x.kg} kg</b>${x.waist ? `<span class="muted"> Â· cintura ${x.waist} cm</span>` : ""}</div><span class="muted">${dispDate(x.d)}</span></div>`).join("")}</div>` : ""}
  </section>
  <section class="card"><div class="lbl">Marcos</div><div class="list">${ms}</div></section>
  <section class="card">
    <div class="lbl">TermÃ´metro do pescoÃ§o (acantose)</div>
    ...
    ${S.neck.length ? `<div class="muted" style="font-size:15px">${S.neck.slice(-6).map(n => `${dispDate(n.d)}: ${n.v}`).join(" Â· ")}</div>` : ""}
  </section>
  <section class="card"><div class="lbl">Ritmo das Ãºltimas 8 semanas</div><div class="row" style="gap:6px;align-items:flex-start">${wkSq}</div><p class="muted" style="font-size:14px">Verde = 5+ dias ativos Â· amarelo = 4 Â· vermelho = menos.</p></section>
...
function suppCatalogHTML() {
  ...
  return a.map(x => `<div class="tile" style="display:flex;flex-direction:column;gap:6px"><div class="row between"><b>${x.n}</b><span class="tag">${x.t === "M" ? "LEVAR AO MÃ‰DICO" : "SUPLEMENTO"}</span></div>
...
function rMais() {
  ...
  <section class="card">
    ...
    <div class="grid2"><div class="tile in"><div class="lbl">TMB</div><div class="mid tabnum">${fmtInt(tg.tmb)}</div></div><div class="tile in"><div class="lbl">Gasto total</div><div class="mid tabnum">${fmtInt(tg.tdee)}</div></div><div class="tile in"><div class="lbl">Meta diÃ¡ria</div><div class="mid tabnum">${fmtInt(tg.kcal)} kcal</div></div><div class="tile in"><div class="lbl">ProteÃ­na</div><div class="mid tabnum">${tg.prot} g</div></div></div>
    <p class="muted" style="font-size:15px">Calculado para ${r1(s.calcWeight)} kg, dÃ©ficit de 1.000 kcal, piso de 1.800. Recalcula sozinho a cada 4 kg de variaÃ§Ã£o.</p>
    <div class="row between"><span class="grow">${esc(S.profile.name)} Â· ${S.profile.height} cm Â· ${S.profile.age} anos</span><button class="btn sm" data-act="profile">${ic("edit")} Editar</button></div>
  </section>
  <section class="card">
    <div class="lbl">Suplementos e remÃ©dios</div>
    <p class="muted" style="font-size:15px">Informativo, sem doses de remÃ©dio. Tudo que Ã© prescriÃ§Ã£o deve ser decidido com seu mÃ©dico. GLP-1 ficou de fora por decisÃ£o sua.</p>
    ...
    <div class="seg"><button class="${UI.supType === "all" ? "on" : ""}" data-act="sup-type" data-v="all">Todos</button><button class="${UI.supType === "S" ? "on" : ""}" data-act="sup-type" data-v="S">Suplementos</button><button class="${UI.supType === "M" ? "on" : ""}" data-act="sup-type" data-v="M">Levar ao mÃ©dico</button></div>
    <div style="display:flex;flex-direction:column;gap:10px" id="supcat">${suppCatalogHTML()}</div>
  </section>
  <section class="card">
    <div class="lbl">AparÃªncia</div>
    ...
    <div class="seg"><button class="${s.logMode === "set" ? "on" : ""}" data-act="logmode" data-v="set">1 toque por sÃ©rie</button><button class="${s.logMode === "end" ? "on" : ""}" data-act="logmode" data-v="end">Ao final</button></div>
  </section>
  <section class="card">
    <div class="lbl">NotificaÃ§Ãµes</div>
    <div class="row between"><span class="grow">Avisar fim do descanso e prÃ³ximo exercÃ­cio</span><button class="tog ${s.notif ? "on" : ""}" data-act="notif" aria-label="notificaÃ§Ãµes"><i></i></button></div>
    ${banner("warn", "info", "Com o app fechado", "Um app web nÃ£o consegue disparar alarme sozinho (05:00, Ã¡gua). Por isso o app gera lembretes para o calendÃ¡rio do seu celular, que tocam mesmo com ele fechado. O aviso de fim de descanso acima sÃ³ funciona com o app aberto.")}
    <button class="btn full" data-act="rem-open">${ic("download")} Criar lembretes no calendÃ¡rio</button>
  </section>
  
  <section class="card">
    <div class="lbl">Dados e backup</div>
    <p class="muted" style="font-size:15px">Camada 1: localStorage. Camada 2: espelho em IndexedDB. Camada 3: arquivo .json, o Ãºnico que sobrevive Ã  troca de celular. ${bk ? `Ãšltimo backup: ${dispDate(bk)} (${diffDays(k, bk)} dias).` : "Nenhum backup exportado ainda."} <span id="persist"></span></p>
  ...
  <section class="card">
    <div class="lbl">Protocolo de recaÃ­da</div>
    <p class="muted" style="font-size:16px"><b>Furou uma refeiÃ§Ã£o:</b> a prÃ³xima Ã© normal. NÃ£o compensa pulando nem treina dobrado.<br><b>Furou um dia:</b> entra como dia ruim e amanhÃ£ segue o plano. Um dia de 4.000 kcal numa semana de 2.000 ainda Ã© dÃ©ficit.<br><b>Furou uma semana:</b> volte pelo menor degrau: sÃ³ nÃ£o beber aÃ§Ãºcar. Os outros hÃ¡bitos voltam depois.<br><b>Nunca se pese apÃ³s furar.</b></p>
  </section>
  <section class="card"><button class="btn danger full" data-act="reset">Apagar todos os dados</button></section>
    <div style="text-align:center; padding:30px 0 20px; color:var(--muted); font-size:12px; font-weight:600; letter-spacing:1px; text-transform:uppercase;">
      âš¡ Powered by Hobert Silva
    </div>`;
  if (navigator.storage && navigator.storage.persisted) navigator.storage.persisted().then(p => { const e = $("#persist"); if (e) e.textContent = p ? "Armazenamento persistente ativo." : "Armazenamento persistente nÃ£o concedido: exporte backups."; });
}
...
function weighSheet() {
  ...
  <p class="muted">Ao acordar, depois do banheiro, antes de comer. Sempre nas mesmas condiÃ§Ãµes.</p>
  ${y ? banner("warn", "info", "Ontem foi um dia difÃ­cil", "O nÃºmero pode estar inflado por sÃ³dio e Ã¡gua. Se puder, espere a prÃ³xima segunda.") : ""}
...
function quickSheet() {
  openSheet(`<h3 class="mid">Adicionar por rÃ³tulo</h3>
  <input class="field" id="qn" placeholder="Nome (ex.: Barra de proteÃ­na)">
  <div class="grid2"><input class="field" id="qk" inputmode="decimal" placeholder="kcal totais"><input class="field" id="qp" inputmode="decimal" placeholder="proteÃ­na (g)"></div>
  <div class="chips">${["CafÃ© da manhÃ£", "AlmoÃ§o", "Lanche", "Jantar", "Ceia"].map(m => `<button class="chip ${UI.meal === m ? "on" : ""}" data-act="meal-sel" data-m="${m}">${m}</button>`).join("")}</div>
...
document.addEventListener("click", e => {
  ...
  switch (a) {
    case "habit": {
      ...
      if (!wasA && isActive(k)) toast(activeInWeek(mondayOf(k)) >= 5 && !wasW ? "Meta da semana cumprida. Isso Ã© ritmo." : "Dia ativo. Este dia conta.");
      break;
    }
    ...
    case "wk-set": {
      ...
      if (s.done && i < dd.sets.length - 1) startRest(st.e.rest, `PrÃ³xima: sÃ©rie ${i + 2} de ${dd.sets.length}`);
      else if (s.done) { const st2 = stepsOf(c.day); const nx = st2[c.i + 1]; if (nx) startRest(60, `PrÃ³ximo: ${nx.t === "warm" ? nx.c.name : findV(nx.e, varId(nx.e))[1]}`); }
      break;
    }
    ...
    case "wk-allset": { const st = stepsOf(c.day)[c.i], dd = wkData(st); dd.sets.forEach(s => s.done = true); save(); notify("ExercÃ­cio registrado", "PrÃ³ximo exercÃ­cio."); wkNext(); break; }
    ...
    case "pain": S.pain.push({ d: today(), day: S.days[today()]?.wk || "", v: +b.dataset.v }); save(); closeSheet(); toast(`Dor ${b.dataset.v}/10 registrada.`); if (painAvg() >= 4) toast("MÃ©dia de dor â‰¥ 4: procure um fisioterapeuta."); break;
    ...
    case "food-add": { const f = FOODS[+b.dataset.i], gm = num($("#g").value); if (gm <= 0) return; const d = DW(k); d.meals.push({ t: Date.now(), n: f[0], g: gm, k: f[1] * gm / 100, p: f[2] * gm / 100, m: UI.meal }); if (f[5] === "Bebidas" && /COM aÃ§Ãºcar/.test(f[0])) d.h.acucar = false; save(); closeSheet(); render(); toast("Adicionado."); break; }
    case "meal-del": { DW(k).meals.splice(+b.dataset.i, 1); save(); render(); break; }
    case "quick": quickSheet(); break;
    case "quick-add": { const kc = num($("#qk").value), pr = num($("#qp").value), nm = $("#qn").value.trim() || "Item de rÃ³tulo"; if (kc <= 0) return toast("Informe as kcal."); DW(k).meals.push({ t: Date.now(), n: nm, g: 0, k: kc, p: pr, m: UI.meal }); save(); closeSheet(); render(); break; }
    case "weigh": weighSheet(); break;
    case "weigh-save": {
      const kg = num($("#wkg").value), waist = num($("#wwaist").value); if (kg < 40 || kg > 300) return toast("Peso invÃ¡lido.");
      ...
      if (Math.abs(kg - S.settings.calcWeight) >= 4) { S.settings.calcWeight = r1(kg); msg = `Peso registrado. Metas recalculadas: ${fmtInt(TG().kcal)} kcal e ${TG().prot} g de proteÃ­na.`; }
    ...
    case "notif": {
      ...
      if (!("Notification" in window)) return toast("Este navegador nÃ£o suporta notificaÃ§Ãµes.");
      Notification.requestPermission().then(p => { S.settings.notif = p === "granted"; save(); rMais(); toast(p === "granted" ? "NotificaÃ§Ãµes ativadas." : "PermissÃ£o negada."); });
      break;
    }
    ...
    case "ia-add": { const d = DW(k); IA.items.forEach(x => d.meals.push({ t: Date.now(), n: "IA: " + x.nome, g: x.g, k: x.k, p: x.p, m: UI.meal })); save(); closeSheet(); render(); toast("Adicionado. Lembre: Ã© estimativa."); haptic(); break; }
...
function rComer() {
    ...
    ext.innerHTML = `
      <div class="card">
        <h3 class="h1" style="font-size:20px;">Meta Fibras & RefeiÃ§Ã£o RÃ¡pida</h3>
        ...
        <div class="alert-glicose" id="agyWalkAlert" style="display:none; background:var(--warn-soft); border-left:4px solid var(--warn); padding:12px; border-radius:8px; margin-top:12px;">
            <strong>Pico GlicÃªmico!</strong> Caminhe 10-15 min para regular.
            <button class="btn sm ghost" style="margin-top:10px" onclick="document.getElementById('agyWalkAlert').style.display='none'">Fiz a caminhada âœ…</button>
        </div>
        <button class="btn solid full" style="margin-top:10px;" id="btnStdLunch">ðŸ± Meu AlmoÃ§o PadrÃ£o (40g P / 10g F)</button>
      </div>
    `;
    vComer.appendChild(ext);

    document.getElementById("btnStdLunch").addEventListener("click", () => {
        d.meals.push({ t: Date.now(), n: "AlmoÃ§o PadrÃ£o (Antigravity)", g: 0, k: 450, p: 40, m: "AlmoÃ§o" });
        d.fiber += 10;
        save();
        rComer(); 
        setTimeout(() => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
      if(new Date().getDay() === 1 && !sessionStorage.getItem("revSeen")) {
          ...
          if(weekNo() >= 8 && rS) rS.innerText = "Semana de ManutenÃ§Ã£o! Teste cargas mÃ¡ximas.";
...
function updateAuraCore() {
    ...
    if(!document.getElementById("auraCoreEl")) {
        ...
        auraContainer.innerHTML = `
            ...
            <div class="aura-msg" id="auraMsgEl">O nÃºcleo estÃ¡ ocioso. Vamos alimentar essa mÃ¡quina?</div>
        `;
    }
    ...
    if(prot >= 160 && hasCardio) {
        ...
        msg.innerText = "Overdrive ativado. Suas fibras e proteÃ­nas estÃ£o otimizadas.";
        msg.style.color = "var(--accent)";
    } else if (prot >= 40 || water > 1000) {
        core.className = "aura-core fed";
        msg.innerText = "O nÃºcleo absorveu energia. Mantenha o fluxo de proteÃ­na e hidrataÃ§Ã£o.";
        msg.style.color = "var(--text)";
    } else {
        core.className = "aura-core";
        msg.innerText = "Sua aura precisa fluir. Tome um copo grande de Ã¡gua agora.";
...
if (typeof rTreino !== 'undefined') {
    ...
    rTreino = function() {
        ...
        if (d && d.skipWk && !UI.pickDay) {
            ...
            if (vTreino) {
                vTreino.innerHTML = `
                <section class="card" style="background:transparent;box-shadow:none;padding:0"><h2 class="h1">Treino</h2><p class="muted">Segunda a sexta Ã s 5h, seguindo a sequÃªncia A a E.</p></section>
                <section class="card" style="text-align:center; padding: 40px 20px;">
                    <div style="font-size:50px; margin-bottom:15px; animation: aura-levitate 3s infinite;">ðŸ›¡ï¸</div>
                    <h3 class="mid" style="font-size: 22px; color: var(--text);">Tudo bem, o foco hoje Ã© na dieta.</h3>
                    <p class="muted" style="margin-top: 10px; line-height: 1.5;">VocÃª marcou que nÃ£o conseguiu ir hoje. O seu treino <b style="color:var(--text);">${L} - ${pName}</b> estÃ¡ guardado para amanhÃ£. A sua sequÃªncia nÃ£o foi quebrada.</p>
            ...
            if (vTreino) {
                ...
                if (cards.length >= 2) {
                    ...
                    if (!btnContainer.querySelector('[data-act="wk-skip"]')) {
                       ...
                       skipBtn.innerText = "NÃ£o consegui ir hoje";
...
function orig_rDeck() {
    ...
    const ab = ABSENCE.find(a => abs >= a.min);
    if (ab) ban.push(banner("acc", "leaf", `${abs} dias sem registro`, ab.txt));
    if (S.cur) ban.push(banner("acc", "play", "Treino em andamento", `VocÃª comeÃ§ou o treino ${S.cur.day}.`, '<div style="margin-top:8px"><button class="btn sm solid" data-act="wk-resume">Continuar treino</button></div>'));
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom: 20px;">
       <div class="card" style="padding:15px; text-align:center; cursor:pointer; background:var(--surface2);" data-go="agua">
          ...
          <div class="lbl">Ãgua Â· ${Math.round(Math.min(1, d.water / S.settings.waterGoal) * 100)}%</div>
       </div>
       ...
       <div class="card" style="padding:15px; text-align:center; cursor:pointer; background:var(--surface2);" data-go="treino">
          <div style="font-size:24px; margin-bottom:5px;">ðŸ‹ï¸â€â™‚ï¸</div>
    ...
    <details class="fold card" style="padding:0 14px; margin-bottom:10px;"><summary><span class="row"><span class="dot ok"></span><span class="lbl" style="font-size:15px; font-weight:600;">HÃ¡bitos DiÃ¡rios (${habitCount(k)}/6)</span></span>${ic("down")}</summary><div class="body" style="display:flex;flex-direction:column;gap:10px">${habitsList}</div></details>
    
    <details class="fold card" style="padding:0 14px; margin-bottom:10px;"><summary><span class="row"><span class="dot sage"></span><span class="lbl" style="font-size:15px; font-weight:600;">SuplementaÃ§Ã£o (${Object.values(d.s).filter(Boolean).length}/${SUPP_BASE.length})</span></span>${ic("down")}</summary><div class="body">${supList}</div></details>
...
function getMetabolicAdvice(k) {
    ...
    if (h >= endOfDay) return { diet: "Dia finalizado. Hora de descansar o sistema digestivo.", water: "Beba Ã¡gua com moderaÃ§Ã£o agora para nÃ£o prejudicar o sono." };
    if (h < 5) return { diet: "Madrugada. Se estiver acordado, mantenha-se hidratado.", water: "Beba Ã¡gua se tiver sede." };
    ...
    if (remainingProt <= 0) {
        dietAdvice = "âœ… Meta de proteÃ­na batida! Se for comer mais tarde, priorize fibras e vegetais.";
        ...
        if (diffHrs < 2) {
            ...
            dietAdvice = "â³ VocÃª comeu hÃ¡ pouco tempo. PrÃ³xima janela anabÃ³lica sugerida: " + nextTimeStr + ". Faltam " + Math.round(remainingProt) + "g no dia.";
        } else if (diffHrs >= 3.5) {
            dietAdvice = "ðŸ”¥ Janela anabÃ³lica aberta! Faltam " + Math.round(remainingProt) + "g no dia. Tente bater " + protPerMeal + "g na prÃ³xima refeiÃ§Ã£o.";
        } else {
            dietAdvice = "â³ Faltam " + Math.round(remainingProt) + "g no dia. SugestÃ£o para a prÃ³xima refeiÃ§Ã£o: " + protPerMeal + "g de proteÃ­na.";
        }
    } else {
        dietAdvice = "ðŸŒ… Primeira refeiÃ§Ã£o do dia? Faltam " + Math.round(remainingProt) + "g. Comece forte com " + protPerMeal + "g de proteÃ­na!";
    }
    ...
    if (remainingWater <= 0) {
        waterAdvice = "âœ… Meta de hidrataÃ§Ã£o batida!";
    } else {
        const mlPerHour = Math.round(remainingWater / hoursLeft);
        if (mlPerHour > 600) {
            waterAdvice = "âš ï¸ VocÃª estÃ¡ desidratado para esse horÃ¡rio. Tome 500ml de Ã¡gua agora.";
        } else {
            waterAdvice = "ðŸ’§ Ritmo ideal: tome aprox. " + mlPerHour + "ml de Ã¡gua por hora atÃ© as " + endOfDay + "h.";
        }
    }
...
orig_rDeck = function() {
    ...
    if(vDeck && auraContainer) {
        ...
        metaBanner.innerHTML = '<div style="font-size:14px; font-weight:600; margin-bottom:4px;">ðŸ§  Dica do NÃºcleo</div><div style="font-size:14px; color:var(--muted); line-height:1.4;">' + advice.diet + ' <br><span style="color:var(--text);">' + advice.water + '</span></div>';
...
orig_rComer = function() {
    ...
    if(vComer) {
        ...
        metaBanner.innerHTML = '<h3 class="h1" style="font-size:18px; display:flex; align-items:center; gap:8px;">ðŸ§  InteligÃªncia MetabÃ³lica</h3><p class="muted" style="margin-top:8px;">' + advice.diet + '</p>';
...
function rIA() {
    ...
    if (!S.settings.gemKey) {
        vIA.innerHTML = `<section class="card" style="background:transparent;box-shadow:none;padding:0;margin-bottom:10px;">
           ...
           <p class="muted">Conecte o cÃ©rebro da sua IA para ela analisar seus treinos, dieta e pratos de comida.</p>
        </section>
        <section class="card">
            <div class="lbl">Chave API do Gemini (fica sÃ³ neste aparelho)</div>
            ...
            <select class="field" id="gemModel" style="margin-bottom:10px;">
                <option value="gemini-2.5-pro" ${'${S.settings.gemModel || "gemini-2.5-flash-lite"}' === 'gemini-2.5-pro' ? 'selected' : ''}>gemini-2.5-pro (AvanÃ§ado - Requer Assinatura Pro)</option>
                <option value="gemini-3.5-flash-lite" ${'${S.settings.gemModel || "gemini-2.5-flash-lite"}' === 'gemini-3.5-flash-lite' ? 'selected' : ''}>gemini-3.5-flash-lite (RÃ¡pido - PadrÃ£o)</option>
    ...
    if (!S.chat || S.chat.length === 0) {
        msgsHTML = `<div style="text-align:center; padding:40px 20px;">
           <div style="font-size:48px; margin-bottom:15px; animation:aura-levitate 3s infinite;">âœ¨</div>
           <h3 class="mid">Sou o seu assistente Aura</h3>
           <p class="muted" style="margin-top:10px;">Eu conheÃ§o a sua dieta, seus treinos e seu histÃ³rico. Me pergunte qualquer coisa, peÃ§a para eu montar seu prato ou tire foto de um rÃ³tulo.</p>
        </div>`;
    } else {
    ...
    <section class="card" style="background:transparent;box-shadow:none;padding:0;margin-bottom:10px;">
       <div class="row between">
           ...
           <div style="display:flex; gap:10px; align-items:center;">
             ...
             <details class="fold" style="margin:0; padding:0; background:transparent;"><summary style="padding:0; margin:0; min-height:auto; font-size:13px; color:var(--accent);">ðŸ”§ Modelo</summary>
...
function buildAuraContext() {
    ...
    return `VocÃª Ã© a Aura, um assistente de saÃºde premium, direto, objetivo e amigÃ¡vel, integrado ao aplicativo Still I Rise.
DADOS DO USUÃRIO HOJE:
- Nome: ${S.profile.name || "UsuÃ¡rio"}
- Meta de Calorias: ${Math.round(tot.k)} / ${tg.kcal} kcal
- Meta de ProteÃ­na: ${Math.round(tot.p)} / ${tg.prot} g
- Ãgua: ${(d.water/1000).toFixed(1)} / ${(S.settings.waterGoal/1000).toFixed(1)} L
- Treino do Dia: ${L}
- Fibras Consumidas: ${d.fiber || 0}g
- HorÃ¡rio Atual: ${new Date().toLocaleTimeString()}

Regras:
1. Responda de forma extremamente concisa, sem enrolaÃ§Ã£o. Use no mÃ¡ximo 2-3 parÃ¡grafos curtos.
...
4. NUNCA diga que Ã© uma IA. Aja como o nÃºcleo inteligente do aplicativo. Mantenha um tom encorajador e prÃ¡tico.`;
}
...
async function askAuraIA(text, file = null) {
    ...
    try {
        ...
        S.chat.push({ role: 'ai', text: "Desculpe, ocorreu um erro na conexÃ£o: " + e.message });
        save();
        rIA();
    }
}
...
(function() {
    document.addEventListener("click", e => {
        ...
        if (act === "ia-cam") {
            ...
            if (confirm("Apagar o histórico e iniciar nova conversa?")) {
                S.chat = [];
                save();
                render();
            }
...
if (remainingProt > 0 && h < endOfDay) {
        ...
        dietAdvice += \<details class="fold" style="margin-top:12px; background:var(--surface2);">
          <summary style="font-size:13px; font-weight:600; color:var(--accent);">💡 Ideias para bater \g de proteína</summary>
          <div class="body" style="font-size:13px; line-height:1.5;">
            <div style="font-weight:600; margin-top:5px; color:var(--text);">🥩 Carnes</div>
            <div style="color:var(--muted);">~\g de Frango desfiado</div>
            <div style="color:var(--muted);">~\g de Patinho (moído)</div>
            <div style="font-weight:600; margin-top:10px; color:var(--text);">🥚 Laticínios e Ovos</div>
            <div style="color:var(--muted);">~\ Ovos inteiros</div>
            <div style="color:var(--muted);">~\g de Queijo Muçarela</div>
            <div style="font-weight:600; margin-top:10px; color:var(--text);">🥤 Suplementos</div>
            <div style="color:var(--muted);">~\g de Whey Protein</div>
          </div>
        </details>\;
