...
function prefill(slot, vid) {
  ...
  if (last) {
    ...
    hint = `Ãšltima vez (${dispDate(last.date)}): ${last.sets.map(s => `${s.kg}Ã—${s.reps}`).join(" Â· ")}` + (inc ? ` â†’ hoje +${inc} kg` : (inAdapt() ? " Â· adaptaÃ§Ã£o: mantenha leve" : ""));
...
function rTreino() {
  ...
  const list = p.ex.map(e => { const v = findV(e, varId(e)); return `<div class="li"><div class="grow"><div style="font-weight:700">${v[1]}</div><div class="muted" style="font-size:14px">${e.sets} Ã— ${e.reps[0] === e.reps[1] ? e.reps[0] : e.reps[0] + "â€“" + e.reps[1]}${e.unit ? " s" : ""} Â· descanso ${e.rest}s${e.inc ? ` Â· +${e.inc} kg` : ""}</div></div><span class="tag">${e.v.length} variaÃ§Ãµes</span></div>`; }).join("");
  ...
  const pa = painAvg();
  $("#v-treino").innerHTML = `
  <section class="card" style="background:transparent;box-shadow:none;padding:0"><h2 class="h1">Treino</h2><p class="muted">Segunda a sexta Ã s 5h. SÃ¡bado e domingo: descanso ativo e caminhada.</p></section>
...
function wkFinish() {
  ...
  toast(`Treino ${L} registrado (${n} exercÃ­cios). Agora 10 min de esteira inclinada.`);
...
function foodListHTML() {
  const q = UI.q.trim().toLowerCase(), norm = s => s.normalize("NFD").replace(/[Ì€-Í¯]/g, "").toLowerCase();
  const nq = norm(q);
  const items = FOODS.map((f, i) => [f, i]).filter(([f]) => (UI.cat === "Todos" || f[5] === UI.cat) && (!nq || norm(f[0]).includes(nq)));
  if (!items.length) return `<p class="muted" style="padding:12px 0">Nada encontrado. Use "Adicionar por rÃ³tulo" abaixo.</p>`;
  return items.slice(0, 60).map(([f, i]) => `<button class="food" data-act="food" data-i="${i}"><div class="grow"><div style="font-weight:600">${esc(f[0])}</div><div class="muted" style="font-size:14px">${f[3]} (${f[4]} g) Â· ${Math.round(f[1] * f[4] / 100)} kcal Â· ${r1(f[2] * f[4] / 100)} g prot</div></div><span class="tag">${f[6]}</span>${ic("plus")}</button>`).join("") + (items.length > 60 ? `<p class="muted" style="font-size:14px;padding:8px 0">Mostrando 60 de ${items.length}. Refine a busca.</p>` : "");
}
function orig_rComer() {
  const k = today(), d = D(k), tg = TG(), tot = dayTotals(k);
  const meals = ["CafÃ© da manhÃ£", "AlmoÃ§o", "Lanche", "Jantar", "Ceia"];
  const groups = meals.map(m => { const it = d.meals.map((x, i) => [x, i]).filter(([x]) => x.m === m); if (!it.length) return ""; const kk = it.reduce((a, [x]) => a + x.k, 0), pp = it.reduce((a, [x]) => a + x.p, 0); return `<div><div class="row between"><span class="lbl">${m}</span><span class="lbl tabnum">${Math.round(kk)} kcal Â· ${Math.round(pp)} g</span></div><div class="list">${it.map(([x, i]) => `<div class="li"><div class="grow"><div style="font-weight:600">${esc(x.n)}</div><div class="muted" style="font-size:14px">${x.g} g Â· ${Math.round(x.k)} kcal Â· ${r1(x.p)} g prot</div></div><button class="circ-btn" style="width:40px;height:40px;color:var(--bad)" data-act="meal-del" data-i="${i}" aria-label="remover">${ic("trash")}</button></div>`).join("")}</div></div>`; }).join("");
  const rem = tg.kcal - tot.k;
  $("#v-comer").innerHTML = `
  <section class="card" style="background:transparent;box-shadow:none;padding:0"><h2 class="h1">Comer</h2><p class="muted">Meta: ${fmtInt(tg.kcal)} kcal e ${tg.prot} g de proteÃ­na. ProteÃ­na de comida, ovos e albumina.</p></section>
  <section class="card">
    <div class="row" style="justify-content:space-around">
    ...
    <p class="muted" style="text-align:center">${rem >= 0 ? `Restam ${fmtInt(rem)} kcal e ${Math.max(0, Math.round(tg.prot - tot.p))} g de proteÃ­na.` : `${fmtInt(-rem)} kcal acima da meta. Um dia acima nÃ£o desfaz uma semana. A prÃ³xima refeiÃ§Ã£o Ã© normal.`}</p>
    ${groups || `<p class="muted" style="text-align:center">Nada registrado hoje.</p>`}
  </section>
  <section class="card">
    <div class="row between"><span class="lbl">Tabela de alimentos (${FOODS.length})</span><span class="muted" style="font-size:13px">T = TACO Â· R = estimativa</span></div>
    ...
    <button class="btn full" data-act="quick">${ic("plus")} Adicionar por rÃ³tulo (kcal e proteÃ­na)</button>
  </section>
  <section class="card">
...
function rMais() {
  ...
  <section class="card">
    ...
    ${banner("warn", "info", "Limite honesto", "Sem servidor de Web Push, o app nÃ£o consegue disparar alarme com ele fechado (05:00, lembrete de Ã¡gua). Para isso use o despertador do celular. As notificaÃ§Ãµes funcionam com o app aberto ou em segundo plano recente.")}
  </section>
  <section class="card">
...
document.addEventListener("click", e => {
  ...
  switch (a) {
    ...
    case "food-add": { const f = FOODS[+b.dataset.i], gm = num($("#g").value); if (gm <= 0) return; const d = DW(k); d.meals.push({ n: f[0], g: gm, k: f[1] * gm / 100, p: f[2] * gm / 100, m: UI.meal }); if (f[5] === "Bebidas" && /COM aÃ§Ãºcar/.test(f[0])) d.h.acucar = false; save(); closeSheet(); render(); toast("Adicionado."); break; }
    case "meal-del": { DW(k).meals.splice(+b.dataset.i, 1); save(); render(); break; }
    case "quick": quickSheet(); break;
    case "quick-add": { const kc = num($("#qk").value), pr = num($("#qp").value), nm = $("#qn").value.trim() || "Item de rÃ³tulo"; if (kc <= 0) return toast("Informe as kcal."); DW(k).meals.push({ n: nm, g: 0, k: kc, p: pr, m: UI.meal }); save(); closeSheet(); render(); break; }
...
document.addEventListener("change", e => {
  ...
  f.text().then(tx => { const o = JSON.parse(tx); if (!o || typeof o !== "object" || !o.profile || !o.days) throw 0; if (!confirm("Importar substitui os dados atuais deste aparelho. Continuar?")) return; S = mergeDefaults(o); save(); applyTheme(); go("deck"); toast("Backup importado."); }).catch(() => toast("Arquivo invÃ¡lido."));
  e.target.value = "";
});
...
function rDeck() {
    ...
    \;
    vDeck.insertBefore(ext, vDeck.firstChild.nextSibling); // After first card
}
...
function rComer() {
    ...
    ext.innerHTML = \
      <div class="card">
        ...
        <div class="row between"><div class="lbl">Fibras (Meta: 30g)</div><div style="font-weight:700">\g</div></div>
        <div class="bar"><i style="width:\%; background:var(--ok)"></i></div>
        <div class="alert-glicose" id="agyWalkAlert">
            <strong>Pico Glicêmico!</strong> Caminhe 10-15 min para regular.
            <button class="btn sm ghost" style="margin-top:10px" onclick="document.getElementById('agyWalkAlert').classList.remove('show')">Fiz a caminhada ✅</button>
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
      if(new Date().getDay() === 1 && !sessionStorage.getItem("revSeen")) {
          ...
          document.getElementById("revW").innerText = "Semana " + weekNo() + "/8";
          if(weekNo() >= 8) document.getElementById("revSug").innerText = "Semana de Manutenção! Teste cargas máximas.";
          document.getElementById("reviewSheet").classList.add("on");
      }
  }, 1000);
});
